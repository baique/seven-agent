...
export class CubismModel {
  ...
  public setParameterValueByIndex(parameterIndex: number, value: number, weight = 1.0): void {
    if (this._notExistParameterValues.has(parameterIndex)) {
      this._notExistParameterValues.set(
        ...
        weight == 1
          ? value
          : this._notExistParameterValues.get(parameterIndex) * (1 - weight) + value * weight,
      )
  ...
  public getDrawableMultiplyColor(drawableIndex: number): CubismTextureColor {
    if (this._drawableMultiplyColors == null) {
      this._drawableMultiplyColors = new Array<CubismTextureColor>(this._model.drawables.count)
  ...
  public getDrawableScreenColor(drawableIndex: number): CubismTextureColor {
    if (this._drawableScreenColors == null) {
      this._drawableScreenColors = new Array<CubismTextureColor>(this._model.drawables.count)
  ...
  public getOffscreenMultiplyColor(offscreenIndex: number): CubismTextureColor {
    if (this._offscreenMultiplyColors == null) {
      const offscreenCount = this._model.offscreens?.count ?? 0
      this._offscreenMultiplyColors = new Array<CubismTextureColor>(offscreenCount)
    ...
    if (!multiplyColors) return new CubismTextureColor()
  ...
  public getOffscreenScreenColor(offscreenIndex: number): CubismTextureColor {
    if (this._offscreenScreenColors == null) {
      const offscreenCount = this._model.offscreens?.count ?? 0
      this._offscreenScreenColors = new Array<CubismTextureColor>(offscreenCount)
    ...
    if (!screenColors) return new CubismTextureColor()
  ...
  public getDrawableAlphaBlend(drawableIndex: number): CubismAlphaBlend {
    ...
    if (this._drawableAlphaBlends[drawableIndex] == CubismAlphaBlend.AlphaBlend_None) {
      const blendModes = this._model.drawables.blendModes
      if (blendModes) {
        this._drawableAlphaBlends[drawableIndex] = (blendModes[drawableIndex] >> 8) & 0xff
      }
    }
  ...
  public isUsingMaskingForOffscreen(): boolean {
    const maskCounts = this._model.offscreens?.maskCounts
    if (!maskCounts) return false
  ...
  public getOffscreenColorBlend(offscreenIndex: number): CubismColorBlend {
    ...
    if (this._offscreenColorBlends[offscreenIndex] == CubismColorBlend.ColorBlend_None) {
      const blendModes = this._model.offscreens?.blendModes
      if (blendModes) {
        this._offscreenColorBlends[offscreenIndex] = blendModes[offscreenIndex] & 0xff
      }
    }
  ...
  public getOffscreenAlphaBlend(offscreenIndex: number): CubismAlphaBlend {
    ...
    if (this._offscreenAlphaBlends[offscreenIndex] == CubismAlphaBlend.AlphaBlend_None) {
      const blendModes = this._model.offscreens?.blendModes
      if (blendModes) {
        this._offscreenAlphaBlends[offscreenIndex] = (blendModes[offscreenIndex] >> 8) & 0xff
      }
    }
  ...
  public getOffscreenOwnerIndices(): Int32Array {
    return this._model.offscreens?.ownerIndices ?? new Int32Array(0)
  }
  ...
  public getOffscreenOpacity(offscreenIndex: number): number {
    const offscreenCount = this._model.offscreens?.count ?? 0
    if (offscreenIndex < 0 || offscreenIndex >= offscreenCount) {
      return 1.0 // オフスクリーンが無いのでスキップ
    }

    return this._model.offscreens?.opacities[offscreenIndex] ?? 1.0
  }
  ...
  public getOffscreenMaskCounts(): Int32Array {
    return this._model.offscreens?.maskCounts ?? new Int32Array(0)
  }
  ...
  public initialize(): void {
    ...
    this._offscreenOpacities = this._model.offscreens?.opacities ?? new Float32Array(0)
  ...
  public constructor(model: Live2DCubismCore.Model) {
    ...
    this._drawableColorBlends = new Array<CubismColorBlend>(model.drawables.count).fill(
      CubismColorBlend.ColorBlend_None,
    )
    this._drawableAlphaBlends = new Array<CubismAlphaBlend>(model.drawables.count).fill(
      CubismAlphaBlend.AlphaBlend_None,
    )

    // Offscreenのカラーブレンドとアルファブレンドの初期化
    const offscreenCount = model.offscreens?.count ?? 0
    this._offscreenColorBlends = new Array<CubismColorBlend>(offscreenCount).fill(
      CubismColorBlend.ColorBlend_None,
    )
    this._offscreenAlphaBlends = new Array<CubismAlphaBlend>(offscreenCount).fill(
      CubismAlphaBlend.AlphaBlend_None,
    )
  }
