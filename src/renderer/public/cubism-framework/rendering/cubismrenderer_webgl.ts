 ...
 */
const s_renderTargetIndexArray: Uint16Array = new Uint16Array([0, 1, 2, 2, 1, 3])
...
export class CubismClippingManager_WebGL extends CubismClippingManager<CubismClippingContext_WebGL> {
  ...
  public setupClippingContext(
    ...
    for (let clipIndex = 0; clipIndex < this._clippingContextListForMask.length; clipIndex++) {
      // 1つのクリッピングマスクに関して
      const cc: CubismClippingContext_WebGL = this._clippingContextListForMask[clipIndex]
    ...
    for (let clipIndex = 0; clipIndex < this._clippingContextListForMask.length; clipIndex++) {
      // --- 実際に1つのマスクを描く ---
      const clipContext: CubismClippingContext_WebGL = this._clippingContextListForMask[clipIndex]
      ...
      for (let i = 0; i < clipDrawCount; i++) {
        ...
        const vertexChanged = model.getDrawableDynamicFlagVertexPositionsDidChange(clipDrawIndex)
        if (!vertexChanged) {
          const id = model.getDrawableId(clipDrawIndex)
          const idStr = id?.getString?.() ?? ''
          const texIdx = model.getDrawableTextureIndex(clipDrawIndex)
          console.log(
            `[MASK SKIP setup] drawable[${clipDrawIndex}] ${idStr} tex${texIdx} - vertex NOT changed!`,
          )
          continue
        }
...
export class CubismRenderer_WebGL extends CubismRenderer {
   ...
   */
  private setupParentOffscreens(model: CubismModel, offscreenCount: number): void {
    ...
    for (let offscreenIndex = 0; offscreenIndex < offscreenCount; ++offscreenIndex) {
      ...
      while (parentIndex != NoParentIndex) {
        for (let i = 0; i < offscreenCount; ++i) {
          const ownerIndex =
            model.getOffscreenOwnerIndices()[this._offscreenList[i].getOffscreenIndex()]
  ...
  public setClippingMaskBufferSize(size: number) {
    ...
    const renderTextureCount: number = this._drawableClippingManager.getRenderTextureCount()
  ...
  public loadShaders(shaderPath: string = null): void {
    ...
    if (
      CubismShaderManager_WebGL.getInstance().getShader(this.gl)._shaderSets.length == 0 ||
  ...
  public doDrawModel(shaderPath: string = null): void {
    ...
    const lastFbo = this.gl.getParameter(this.gl.FRAMEBUFFER_BINDING) as WebGLFramebuffer
  ...
  public drawDrawable(drawableIndex: number, rootFbo: WebGLFramebuffer): void {
    ...
    const model = this.getModel()
    const texIdx = model.getDrawableTextureIndex(drawableIndex)
    // DEBUG: 描画開始ログ
    // if (texIdx === 0 || texIdx === 1) {
    //   const id = model.getDrawableId(drawableIndex)
    //   const idStr = id?.getString?.() ?? ''
    //   console.log(`[DRAW] drawable[${drawableIndex}] ${idStr} tex${texIdx} ✓ entering drawDrawable`)
    // }
    ...
    if (clipContext != null && this.isUsingHighPrecisionMask()) {
...

      {
        ...
        for (let index = 0; index < clipDrawCount; index++) {
          ...
          // 頂点情報が更新されておらず、信頼性がない場合は描画をパスする
          const vertexChanged =
            this._model.getDrawableDynamicFlagVertexPositionsDidChange(clipDrawIndex)
          if (!vertexChanged) {
            const id = this._model.getDrawableId(clipDrawIndex)
            const idStr = id?.getString?.() ?? ''
            const texIdx = this._model.getDrawableTextureIndex(clipDrawIndex)
            console.log(
              `[MASK SKIP] drawable[${clipDrawIndex}] ${idStr} tex${texIdx} - vertex NOT changed, skipping mask draw!`,
            )
            continue
          }
  ...
  public drawMeshWebGL(model: Readonly<CubismModel>, index: number): void {
    const texIdx = model.getDrawableTextureIndex(index)
    const isMask = this.isGeneratingMask()
...

    {
      ...
      // DEBUG: 追踪texture_00的特定drawable (69, 70, 71是ArtMesh106-108)
      // if (texIdx === 0 && !isMask && (index === 69 || index === 70 || index === 71)) {
      //   const id = model.getDrawableId(index)
      //   const idStr = id?.getString?.() ?? ''
      //   console.log(
      //     `[DRAW EXEC] drawable[${index}] ${idStr} tex${texIdx} ✓ drawElements(${indexCount} indices)`,
      //   )
      // }
  ...
  submitDrawToParentOffscreen(objectIndex: number, objectType: DrawableObjectType): void {
    ...
    const currentOwnerIndex =
      this.getModel().getOffscreenOwnerIndices()[this._currentOffscreen.getOffscreenIndex()]
  ...
  public beforeDrawModelRenderTarget(): void {
    ...
    for (let i = 0; i < this._modelRenderTargets.length; ++i) {
      if (
        this._modelRenderTargets[i].getBufferWidth() != this._modelRenderTargetWidth ||
        this._modelRenderTargets[i].getBufferHeight() != this._modelRenderTargetHeight
      ) {
   ...
   */
  public setClippingContextBufferForDrawable(clip: CubismClippingContext_WebGL): void {
   ...
   */
  public setClippingContextBufferForOffscreen(clip: CubismClippingContext_WebGL): void {
  ...
  public startUp(gl: WebGLRenderingContext | WebGL2RenderingContext): void {
    ...
    if (this._model.isBlendModeEnabled()) {
      ...
      if (offscreenCount > 0) {
        this._offscreenList = new Array<CubismOffscreenRenderTarget_WebGL>(offscreenCount)
        for (let offscreenIndex = 0; offscreenIndex < offscreenCount; ++offscreenIndex) {
