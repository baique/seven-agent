...
> implements ICubismClippingManager {
   ...
   */
  public initializeForDrawable(model: CubismModel, renderTextureCount: number): void {
    // レンダーテクスチャの合計枚数の設定
   ...
   */
  public initializeForOffscreen(model: CubismModel, maskBufferCount: number): void {
  ...
  public findSameClip(drawableMasks: Int32Array, drawableMaskCounts: number): T_ClippingContext {
    ...
    for (let i = 0; i < this._clippingContextListForMask.length; i++) {
      const clippingContext: T_ClippingContext = this._clippingContextListForMask[i]
  ...
  public setupMatrixForHighPrecision(model: CubismModel, isRightHanded: boolean): void {
    ...
    if (usingClipCount > 0) {
      ...
      for (let clipIndex = 0; clipIndex < this._clippingContextListForMask.length; clipIndex++) {
        // --- 実際に１つのマスクを描く ---
        const clipContext: T_ClippingContext = this._clippingContextListForMask[clipIndex]
  ...
  public setupLayoutBounds(usingClipCount: number): void {
    ...
    if (usingClipCount <= 0 || usingClipCount > useClippingMaskMaxCount) {
      ...
      for (let index = 0; index < this._clippingContextListForMask.length; index++) {
        const clipContext: T_ClippingContext = this._clippingContextListForMask[index]
    ...
    for (
      ...
      for (let channelIndex = 0; channelIndex < ColorChannelCount; channelIndex++) {
        ...
        if (layoutCount == 0) {
          ...
          // 全てをそのまま使う
          const clipContext: T_ClippingContext = this._clippingContextListForMask[curClipIndex++]
