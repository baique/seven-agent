...
function correctEndPoint(
  ...
  switch (motionData.segments[segmentIndex].segmentType as CubismMotionSegmentType) {
...
export class CubismMotion extends ACubismMotion {
  ...
  public doUpdateParameters(
    ...
    for (
      ;
      c < this._motionData.curveCount &&
      curves[c].type == CubismMotionCurveTarget.CubismMotionCurveTarget_Parameter;
      ++c
    ) {
...

    {
      if (eyeBlinkValue != Number.MAX_VALUE) {
        for (let i = 0; i < this._eyeBlinkParameterIds.length && i < maxTargetSize; ++i) {
          const sourceValue: number = model.getParameterValueById(this._eyeBlinkParameterIds[i])
      ...
      if (lipSyncValue != Number.MAX_VALUE) {
        for (let i = 0; i < this._lipSyncParameterIds.length && i < maxTargetSize; ++i) {
          const sourceValue: number = model.getParameterValueById(this._lipSyncParameterIds[i])
    ...
    for (
      ;
      c < this._motionData.curveCount &&
      curves[c].type == CubismMotionCurveTarget.CubismMotionCurveTarget_PartOpacity;
      ++c
    ) {
      // Find parameter index.
      parameterIndex = model.getParameterIndex(curves[c].id)
  ...
  public parse(
    ...
    for (let curveCount = 0; curveCount < this._motionData.curveCount; ++curveCount) {
      ...
      this._motionData.curves[curveCount].fadeInTime = json.isExistMotionCurveFadeInTime(curveCount)
        ? json.getMotionCurveFadeInTime(curveCount)
        : -1.0
      this._motionData.curves[curveCount].fadeOutTime = json.isExistMotionCurveFadeOutTime(
        curveCount,
      )
      ...
      for (
        ...
        if (segmentPosition == 0) {
          ...
          this._motionData.points[totalPointCount].time = json.getMotionCurveSegment(
            curveCount,
            segmentPosition,
          )
          this._motionData.points[totalPointCount].value = json.getMotionCurveSegment(
            curveCount,
            segmentPosition + 1,
          )
          ...
          // 保护性检查：确保 totalPointCount > 0
          if (totalPointCount === 0) {
            console.warn(
              '[CubismMotion] Invalid motion data: totalPointCount is 0 when segmentPosition > 0',
            )
            // 跳过这个 curve
            break
          }
        ...
        switch (segmentType) {
          ...
          case CubismMotionSegmentType.CubismMotionSegmentType_Bezier: {
            ...
            this._motionData.points[totalPointCount + 1].time = json.getMotionCurveSegment(
              curveCount,
              segmentPosition + 3,
            )
            this._motionData.points[totalPointCount + 1].value = json.getMotionCurveSegment(
              curveCount,
              segmentPosition + 4,
            )

            this._motionData.points[totalPointCount + 2].time = json.getMotionCurveSegment(
              curveCount,
              segmentPosition + 5,
            )
            this._motionData.points[totalPointCount + 2].value = json.getMotionCurveSegment(
              curveCount,
              segmentPosition + 6,
            )
   ...
   */
  public getFiredEvent(beforeCheckTimeSeconds: number, motionTimeSeconds: number): Array<string> {
