...
export class CubismModelSettingJson extends ICubismModelSetting {
  ...
  public getLayoutMap(outLayoutMap: Map<string, number>): boolean {
    ...
    const map: Map<string, Value> = this.getJson().getRoot().getValueByString(this.layout).getMap()
  ...
  public getEyeBlinkParameterCount(): number {
    ...
    for (let i = 0; i < this._jsonValue[FrequestNode.FrequestNode_Groups].getSize(); i++) {
      const refI: Value = this._jsonValue[FrequestNode.FrequestNode_Groups].getValueByIndex(i)
  ...
  protected isExistMotionGroupName(groupName: string): boolean {
    const node: Value =
      this._jsonValue[FrequestNode.FrequestNode_Motions].getValueByString(groupName)
    return !node.isNull() && !node.isError()
  }
